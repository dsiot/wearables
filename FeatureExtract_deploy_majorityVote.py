...
file = open('FeaturesSetSVM.csv','w')

outputWriter = csv.writer(file)
if os.stat("FeaturesSetSVM.csv").st_size == 0:
    ...
    if(countClassify == 4800):
        ...
        for i in range(2):
            ...
            if(out_ann[i,0] < out_ann[i,1]):
                ...
                stress[i] = stress[i] + 1
        print 'relax vote = '+str(relax)
        print 'stress vote = ' + str(stress)
        #Take a majority vote to decide classfication result
        for i in range(2):
            if(relax[i] > stress[i]):
                #relax
                label[i,1] = 1
            elif(relax[i]< stress[i]):
                #stress
                label[i,1] = 0
